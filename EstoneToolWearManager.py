...
"""
def update_database(dictionary, update_type, chosen_tool):
    ...
    if update_type == 'lifetime':
        ...
        with open('ToolDistanceDB.json', 'w') as json_file_object:
            json.dump(combined_dictionaries, json_file_object, indent=2, sort_keys=True)
    tool_wear_manager_display(chosen_tool)

"""
defines the function that handles calculation of lifetime_distance_traveled by incrementing each tool's distance in the dictionary 'lifetime_distance_traveled' by the
corresponding tool in 'executed_distance_traveled'.
"""
def lifetime_distance_traveled_calculation(executed_distance_traveled, chosen_tool):
    combined_dictionaries = create_combined_dictionaries()
    lifetime_distance_traveled = combined_dictionaries.get('lifetime_distance_traveled')
    ...
    tool_wear_calculation(lifetime_distance_traveled, chosen_tool)
    acknowledge_window_for_running_importdata_script(executed_distance_traveled, lifetime_distance_traveled, chosen_tool)

"""
Defines the function that calculates tool_wear as a ratio between lifetime_distance_traveled and ideal_distance_traveled
"""
def tool_wear_calculation(dictionary, chosen_tool):
    combined_dictionaries = create_combined_dictionaries()
    ideal_distance_traveled = combined_dictionaries.get('ideal_distance_traveled')
    tool_wear_dictionary = combined_dictionaries.get('tool_wear')
    ...
    update_database(tool_wear_dictionary, update_type, chosen_tool)
...
"""
def executed_tool_change(chosen_tool):
    if chosen_tool.get() == 'Select tool...':
        messagebox.showerror('Executed Tool Change', 'No tool selected!')
    else:
        response = messagebox.askokcancel("Executed Tool Change", "Was " + str(chosen_tool.get()) + " replaced with new parts?")
        if response == 1:
            messagebox.showinfo("Executed Tool Change", str(chosen_tool.get()) + "\'s Lifetime Distance Traveled was updated to 0.0 ft.")
            combined_dictionaries = create_combined_dictionaries()
            lifetime_distance_traveled = combined_dictionaries.get('lifetime_distance_traveled')
            temporary_iterable_dictionary = {chosen_tool.get():0.0}
            lifetime_distance_traveled.update(temporary_iterable_dictionary)
            update_type = 'lifetime'
            update_database(lifetime_distance_traveled, update_type, chosen_tool)
...
"""
def calculate_executed_distance_traveled(raw_data_filepath, chosen_tool):
    ...
    if response == 1:
        try:
            ...
            lifetime_distance_traveled_calculation(executed_distance_traveled, chosen_tool)
...
"""
def update_ideal_distance_traveled(chosen_tool):
    if chosen_tool.get() == "Select tool...":
        messagebox.showerror('Update Ideal Distance Traveled', "No tool selected!")
    else:
        response = messagebox.askokcancel('Update Ideal Distance Traveled', "Are you sure you want to update the Ideal Distance Traveled for tool " + str(chosen_tool.get()) + ' ?')
        ideal_distance_traveled_manager(response, chosen_tool)

"""
Opens a new window if the user wishes to update the Ideal distance traveled for an individual tool.
"""
def ideal_distance_traveled_manager(response, chosen_tool):
    if response == 1:
        ideal_distance_traveled_manager_window = Toplevel(root)
        ideal_distance_traveled_manager_window.title('Update Ideal Distance Traveled for tool ' + str(chosen_tool.get()))
        info_label = Label(ideal_distance_traveled_manager_window, text="Enter New Ideal Distance Traveled for tool " + str(chosen_tool.get()) + " in feet (ft)", bd=3, anchor=CENTER)
        ...
        update_button = Button(ideal_distance_traveled_manager_window, text='Update', command=lambda: change_ideal_distance_traveled(new_ideal_distance_traveled_entry_field, chosen_tool, response, ideal_distance_traveled_manager_window))
...
"""
def change_ideal_distance_traveled(new_ideal_distance_traveled_entry_field, chosen_tool, response, ideal_distance_traveled_manager_window):
    combined_dictionaries = create_combined_dictionaries()
    lifetime_distance_traveled = combined_dictionaries.get('lifetime_distance_traveled')
    ideal_distance_traveled = combined_dictionaries.get('ideal_distance_traveled')
    ...
    try:
        ...
        temporary_iterable_dictionary = {chosen_tool.get():new_ideal_distance_traveled_for_selected_tool}
        ideal_distance_traveled.update(temporary_iterable_dictionary)
        update_type = 'ideal'
        update_database(ideal_distance_traveled, update_type, chosen_tool)
        tool_wear_calculation(lifetime_distance_traveled)
...
"""
The place where the user picks the raw data to use for calculating executed_distance_traveled. This function also updates the label label_for_the_selected_raw_data_file_directory with
the file directory that was selected.
"""
...
"""
def acknowledge_window_for_running_importdata_script(executed_distance_traveled, lifetime_distance_traveled, chosen_tool):
    combined_dictionaries = create_combined_dictionaries()
    ...
    acknowledge_button = Button(acknowledgement_window_for_running_importdata_script, text='Acknowledge', padx=5, pady=5, bd=2, command=lambda: update_and_destroy(updated_lifetime_distance_traveled, update_type, acknowledgement_window_for_running_importdata_script, chosen_tool))
    acknowledge_button.grid(column=2, row=final_row_number)
    acknowledgement_window_for_running_importdata_script.mainloop()

def update_and_destroy(updated_lifetime_distance_traveled, update_type, acknowledgement_window_for_running_importdata_script, chosen_tool):
    acknowledgement_window_for_running_importdata_script.destroy()
    update_database(updated_lifetime_distance_traveled, update_type, chosen_tool)

"""
'tool_wear_manager_display' and 'create_text_for_label' are the two functions that populate the labels shown in the tool_wear_manager_display_frame. The purpose of the
'tool_wear_manager_display' function is to define the labels, and place them on the screen. 'tool_wear_manager_display' is only called when the operator selects a choice in the OptionMenu
...
this GUI are either vanilla python functions or tkinter "Callbacks," the data type of 'chosen_tool' changes between a 'str' clas and a 'tkinter.string'. When 'chosen_tool' is a
...
'chosen_tool,' and points the function to handle the string manipulation and concatonation accordingly.
"""
def tool_wear_manager_display(chosen_tool):
    label_text = create_text_for_label(chosen_tool)
    string_comparator = str()
    if type(chosen_tool) == type(string_comparator):
        if chosen_tool == 'Select tool...':
            pass
        else:
...
def create_text_for_label(chosen_tool):
    string_comparator = str()
    label_text = []
    combined_dictionaries = create_combined_dictionaries()
    lifetime_distance_traveled = combined_dictionaries.get('lifetime_distance_traveled')
    ...
    tool_wear_dictionary = combined_dictionaries.get('tool_wear')
    if type(chosen_tool) == type(string_comparator):
        label_text.append(chosen_tool)
        if chosen_tool not in lifetime_distance_traveled.keys():
            label_text.append(' ')
        else:
            lifetime_distance_for_tool = str(lifetime_distance_traveled.get(chosen_tool)) + ' ft.'
            lifetime_distance_for_tool_truncated = lifetime_distance_for_tool[:lifetime_distance_for_tool.find('.') + 3]
            label_text.append('Lifetime Distance Traveled for ' + chosen_tool + ' : ' + lifetime_distance_for_tool_truncated)
        if chosen_tool not in ideal_distance_traveled.keys():
            label_text.append(' ')
        else:
            ideal_distance_for_tool = str(ideal_distance_traveled.get(chosen_tool)) + ' ft.'
            label_text.append('Ideal Distance Traveled for ' + chosen_tool + ' : ' + ideal_distance_for_tool)
        if chosen_tool not in tool_wear_dictionary.keys():
            label_text.append(' ')
        else:
            tool_wear_percentage = tool_wear_dictionary.get(chosen_tool) * 100
            tool_wear_percentage_string = str(tool_wear_percentage)
            tool_wear_percentage_truncated_string = tool_wear_percentage_string[:3]  + ' %'
            label_text.append('Tool Wear percentage for ' + chosen_tool + ' : ' + tool_wear_percentage_truncated_string)
        return label_text
    else:
        label_text.append(chosen_tool.get())
        if chosen_tool.get() not in lifetime_distance_traveled.keys():
            label_text.append(' ')
        else:
            lifetime_distance_for_tool = str(lifetime_distance_traveled.get(chosen_tool.get())) + ' ft.'
            lifetime_distance_for_tool_truncated = lifetime_distance_for_tool[:lifetime_distance_for_tool.find('.') + 3]
            label_text.append('Lifetime Distance Traveled for ' + chosen_tool.get() + ' : ' + lifetime_distance_for_tool_truncated)
        if chosen_tool.get() not in ideal_distance_traveled.keys():
            label_text.append(' ')
        else:
            ideal_distance_for_tool = str(ideal_distance_traveled.get(chosen_tool.get())) + ' ft.'
            label_text.append('Ideal Distance Traveled for ' + chosen_tool.get() + ' : ' + ideal_distance_for_tool)
        if chosen_tool.get() not in tool_wear_dictionary.keys():
            label_text.append(' ')
        else:
            tool_wear_percentage = tool_wear_dictionary.get(chosen_tool.get()) * 100
            tool_wear_percentage_string = str(tool_wear_percentage)
            tool_wear_percentage_truncated_string = tool_wear_percentage_string[:3]  + ' %'
            label_text.append('Tool Wear percentage for ' + chosen_tool.get() + ' : ' + tool_wear_percentage_truncated_string)
        return label_text
...
"""
Initializes the tkinter variable for our chosen_tool dropdown menu.
"""
chosen_tool = StringVar()
chosen_tool.set("Select tool...")
...
"""
button_that_clears_lifetime_distance_traveled = Button(tool_wear_manager_frame, text="Executed Tool Change", command=lambda: executed_tool_change(chosen_tool), padx=5, pady=5)
...
"""
button_that_calculates_executed_distance_traveled = Button(calculate_executed_distance_traveled_frame, text="Calculate Executed Distance Traveled", command=lambda: calculate_executed_distance_traveled(raw_data_filepath, chosen_tool), padx=5, pady=5)
...
"""
button_that_updates_ideal_distance_traveled = Button(tool_wear_manager_frame, text='Update Ideal Distance Traveled', command=lambda: update_ideal_distance_traveled(chosen_tool), padx=5, pady=5)
...
"""
tool_selection = OptionMenu(tool_wear_manager_frame, chosen_tool, 'E001', 'E002', 'E003', 'E004', 'E005', 'E006', 'E007', 'E008', 'E009', 'E010', 'E011', 'E012', 'E013', 'E014', 'E015', 'E016', 'E017', 'E018', command=tool_wear_manager_display)
