...
"""
def segment(data_line_index, data_in_data_line, new_cut_index_number, tool_being_cut_with, executed_distance_traveled, toolpathdata):
    if data_line_index >= new_cut_index_number + 2 and "Segment" in data_in_data_line[0]:
        ...
        if z1 != zo:
            pass_in_or_out_of_material(xo, yo, zo, x1, y1, z1, tool_being_cut_with, executed_distance_traveled)
        elif z1 > 0 and zo > 0:
            pass
        else:
            segment_return = (((((xo - x1) ** 2) + ((yo - y1) ** 2) + ((zo - z1) ** 2)) ** 0.5) / 25.4) / 12
...
"""
def arc(data_line_index, data_in_data_line, new_cut_index_number, tool_being_cut_with, executed_distance_traveled):
    if data_line_index >= new_cut_index_number + 2 and "Arc" in data_in_data_line[0]:
        ...
        arc_radius = ((((xend - xcent) ** 2) + ((yend - ycent) ** 2)) ** 0.5) / 25.4
        arc_length = ((3.141592 * arc_radius) / 2) / 12
...
of the material.
"""
def pass_in_or_out_of_material(xo, yo, zo, x1, y1, z1, tool_being_cut_with, executed_distance_traveled):
    ...
    segment_return = (((((x - coordinate_list_of_point_below_surface_of_material[0]) ** 2) + ((y - coordinate_list_of_point_below_surface_of_material[1]) ** 2) + ((z - coordinate_list_of_point_below_surface_of_material[2]) ** 2)) ** 0.5) / 25.4) / 12
    executed_distance_traveled[tool_being_cut_with] += segment_return
...
We can use this script file to calculate executed_distance_traveled, for use in tool wear. It also performs simple logic to detect whether the program should add a new tool to the executed_distance_traveled dictionary and calculate it's distance, or simply calculate the
...
def datacleaning(raw_data_filepath):
    ...
    for data_line_index, data_in_data_line in enumerate(toolpathdata):
        if data_in_data_line[0] == "Tool":
            ...
            segment(data_line_index, data_in_data_line, new_cut_index_number, tool_being_cut_with, executed_distance_traveled, toolpathdata)
            arc(data_line_index, data_in_data_line, new_cut_index_number, tool_being_cut_with, executed_distance_traveled)
